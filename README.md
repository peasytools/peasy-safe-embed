...
[![npm](https://img.shields.io/npm/v/peasy-safe-embed)](https://www.npmjs.com/package/peasy-safe-embed)
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](https://opensource.org/licenses/MIT)
[![Zero Dependencies](https://img.shields.io/badge/dependencies-0-brightgreen)](https://www.npmjs.com/package/peasy-safe-embed)
[![Size](https://img.shields.io/badge/gzipped-<8KB-green)](https://bundlephobia.com/package/peasy-safe-embed)

Embed **PeasySafe** widgets on any website or blog. Security tools — password generation, hashing, encryption, and checks. Each widget is completely isolated via **Shadow DOM**, requires **zero dependencies**, and supports **3 built-in themes** (light, dark, sepia). The self-executing script finds all `data-peasy-safe` elements on the page and renders interactive widgets automatically, including support for dynamically added elements in single-page applications via MutationObserver.

PeasySafe is part of the [Peasy Tools](https://peasytools.com) ecosystem, providing 11 widget types for embedding file format information, tool cards, conversion guides, glossary terms, search boxes, and interactive tools directly into your pages.

> **Try the interactive widget builder at [widget.peasysafe.com](https://widget.peasysafe.com)** -- build, preview, and copy embed code for any widget type.

<p align="center">
  <img src="demo.gif" alt="peasy-safe-embed demo -- embed peasysafe widgets with Shadow DOM isolation and theme support" width="800">
</p>
...
<div data-peasy-safe="tool"
  data-slug="password-generator"
  data-theme="light">
</div>

<!-- 2. Load the embed script once, anywhere on the page -->
<script src="https://cdn.jsdelivr.net/npm/peasy-safe-embed@1/dist/embed.min.js"></script>
```

That's it. The script will find all `data-peasy-safe` elements, fetch data from the PeasySafe API, and render each widget with full Shadow DOM style isolation.

## Widget Types

PeasySafe supports 11 widget types -- 11 common types available on all Peasy sites.

| Type | Description | Usage |
|------|-------------|-------|
| `format` | File format info card with specs, MIME type, and use cases | `<div data-peasy-safe="format" data-slug="..."></div>` |
| `tool` | Tool card with description and direct launch link | `<div data-peasy-safe="tool" data-slug="..."></div>` |
| `convert` | Conversion card showing input/output format pair | `<div data-peasy-safe="convert" data-slug="..."></div>` |
| `compare` | Side-by-side comparison of two formats or tools | `<div data-peasy-safe="compare" data-slug="..."></div>` |
| `glossary` | Glossary term tooltip with definition on hover | `<div data-peasy-safe="glossary" data-slug="..."></div>` |
| `gallery` | Grid gallery of available tools for a category | `<div data-peasy-safe="gallery" data-slug="..."></div>` |
| `guide` | Guide/tutorial card with summary and read-more link | `<div data-peasy-safe="guide" data-slug="..."></div>` |
| `usecase` | Use-case card showing when and why to use a tool | `<div data-peasy-safe="usecase" data-slug="..."></div>` |
| `badge` | Inline badge showing format type or tool category | `<div data-peasy-safe="badge" data-slug="..."></div>` |
| `search` | Search box with autocomplete for tools and formats | `<div data-peasy-safe="search" data-slug="..."></div>` |
| `interactive` | Embedded interactive tool via iframe | `<div data-peasy-safe="interactive" data-slug="..."></div>` |
...
|-----------|--------|---------|-------------|
| `data-peasy-safe` | format, tool, convert, compare, glossary, gallery, guide, usecase, badge, search, interactive | *required* | Widget type to render |
...
<div data-peasy-safe="tool" data-slug="password-generator" data-theme="light"></div>

<!-- Dark theme -- dark background, light text -->
<div data-peasy-safe="tool" data-slug="password-generator" data-theme="dark"></div>

<!-- Sepia theme -- warm, paper-like appearance -->
<div data-peasy-safe="tool" data-slug="password-generator" data-theme="sepia"></div>

<!-- Auto theme -- follows prefers-color-scheme -->
<div data-peasy-safe="tool" data-slug="password-generator" data-theme="auto"></div>
```

Each theme sets CSS custom properties inside the Shadow DOM. The accent color (`#64748B`) is consistent across all themes, providing brand recognition for PeasySafe.
...
```html
<script type="module">
  import 'https://cdn.jsdelivr.net/npm/peasy-safe-embed@1/dist/embed.esm.js';
</script>
```
...
```html
<!-- PeasySafe format info card with light theme -->
<div data-peasy-safe="format" data-slug="password-generator" data-theme="light"></div>
<script src="https://cdn.jsdelivr.net/npm/peasy-safe-embed@1/dist/embed.min.js"></script>
```
...
```html
<!-- Search across PeasySafe tools and formats -->
<div data-peasy-safe="search"
  data-placeholder="Search PeasySafe tools..."
  data-theme="light">
</div>
<script src="https://cdn.jsdelivr.net/npm/peasy-safe-embed@1/dist/embed.min.js"></script>
```
...
```html
<p>This file is in <div data-peasy-safe="badge" data-slug="password-generator"></div> format.</p>
<script src="https://cdn.jsdelivr.net/npm/peasy-safe-embed@1/dist/embed.min.js"></script>
```
...
```html
<!-- Embed an interactive PeasySafe tool -->
<div data-peasy-safe="interactive"
  data-slug="safe/example-tool"
  data-height="500"
  data-theme="light">
</div>
<script src="https://cdn.jsdelivr.net/npm/peasy-safe-embed@1/dist/embed.min.js"></script>
...
> **Note:** The `data-slug` value uses the format `category-slug/tool-slug`. Find available tools at [peasysafe.com/api/v1/tools/](https://peasysafe.com/api/v1/tools/).
...
```html
<div data-peasy-safe="glossary" data-slug="lossless" data-theme="light"></div>
<script src="https://cdn.jsdelivr.net/npm/peasy-safe-embed@1/dist/embed.min.js"></script>
```
...
- **CORS**: PeasySafe API has CORS enabled for all origins
...
- **Powered by**: Each widget includes a subtle "PeasySafe" backlink in the footer

## Learn More

Visit [peasysafe.com](https://peasysafe.com) for the full PeasySafe experience.

- **All Tools**: [peasysafe.com](https://peasysafe.com)
- **Formats**: [peasysafe.com/formats/](https://peasysafe.com/formats/)
- **Glossary**: [peasysafe.com/glossary/](https://peasysafe.com/glossary/)
- **Guides**: [peasysafe.com/guides/](https://peasysafe.com/guides/)
- **API Documentation**: [peasysafe.com/developers/](https://peasysafe.com/developers/)
- **OpenAPI Spec**: [peasysafe.com/api/openapi.json](https://peasysafe.com/api/openapi.json)
- **Widget Builder**: [widget.peasysafe.com](https://widget.peasysafe.com)
- **npm Package**: [npmjs.com/package/peasy-safe-embed](https://www.npmjs.com/package/peasy-safe-embed)
...
Embed [PeasySafe](https://peasysafe.com) widgets on any website with [peasy-safe-embed](https://widget.peasysafe.com):

```html
<script src="https://cdn.jsdelivr.net/npm/peasy-safe-embed@1/dist/embed.min.js"></script>
<div data-peasysafe="entity" data-slug="example"></div>
```

Zero dependencies · Shadow DOM · 4 themes (light/dark/sepia/auto) · [Widget docs](https://widget.peasysafe.com)
